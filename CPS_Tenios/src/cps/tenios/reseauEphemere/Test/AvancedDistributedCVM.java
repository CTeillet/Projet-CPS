...
public class AvancedDistributedCVM extends AbstractDistributedCVM {
	...
	public AvancedDistributedCVM(String[] args) throws Exception {
		super(args);
		...
		if (AbstractCVM.getThisJVMURI().equals(jvmGestionnaire)) {
			...
			for(i=0; i < jvmComposantURI.length; i++) {
				if (AbstractCVM.getThisJVMURI().equals(jvmComposantURI[i])) {
					...
					uri[i] = AbstractComponent.createComponent(kind,
							new Object[] {composantsURI[i], addresses[i], (int) (Math.random() * 25), (int) (Math.random() * 25), 15.});
	...
	public static void main(String[] args) {
		...
		try {
			AvancedDistributedCVM dcvm = new AvancedDistributedCVM(args);
